...
// Features: Dark mode, Labels, Priorities, Dependencies, Quick-add NLP,
...
const app = {
  ...
  currentView: 'home', selectedTasks: new Set(), cmdSelectedIndex: 0, selectedProjectId: null, projectViewMode: 'list',
  theme: 'light', draggedSubtaskId: null,
  currentUserId: 'u1',
  boardColumns: [],
  collapsedTasks: new Set(), // accordion state for tree view (persisted)
  expandedTasks: new Set(), // explicitly expanded tasks (we track expanded since default=collapsed)
  // Feature: Undo
  undoStack: [],
  // Feature: Sort
  sortPref: 'status',
  ...
  // ===== ID GENERATION (PocketBase-compatible: 15 lowercase alphanumeric) =====
  generateId() {
  ...
  migrateOldSubtasks() {
    ...
    this.tasks.forEach(t => {
      // Ensure parentId field exists
      if (t.parentId === undefined) t.parentId = '';
      // Convert old subtasks array to real tasks
      if (t.subtasks && t.subtasks.length > 0) {
        t.subtasks.forEach((st, i) => {
          // Only migrate if this subtask isn't already in the tasks array
  ...
  async _syncToPb() {
    ...
    const collections = [
      {
        name: 'projects', data: this.projects,
        toRecord: r => ({ name: r.name, color: r.color, description: r.description })
      },
      {
        name: 'labels', data: this.labels,
        toRecord: r => ({ name: r.name, color: r.color })
      },
      {
    ...
    // Sync user profile updates (name, role, color) — no create/delete from client
  ...
  renderMyTasks() {
    ...
    const renderTaskRow = (t, level, parentTask) => {
      ...
      const isCollapsed = !this.expandedTasks.has(t.id);
  ...
  onListDrop(e, targetId, targetLevel) {
    ...
    if (isNest) {
      ...
      this.expandedTasks.add(target.id);
  ...
  renderBoard() {
    ...
    board.innerHTML = this.boardColumns.map(col => {
      ...
      const cardsHtml = tasks.length ? tasks.map(t => {
        ...
        const bsel = this.selectedTasks.has(t.id);
  ...
  clearSelection() { this.selectedTasks.clear(); if (this.currentView === 'my-tasks') this.renderMyTasks(); else if (this.currentView === 'board') this.renderBoard(); this.updateBulkBar(); },

  updateBulkBar() {
    const bar = document.getElementById('bulk-bar');
    const show = this.selectedTasks.size > 0;
    bar.classList.toggle('show', show);
    document.getElementById('bulk-count-num').textContent = this.selectedTasks.size;
  },
  ...
  bulkDelete() {
    if (!confirm(`Delete ${this.selectedTasks.size} tasks?`)) return;
    this.pushUndo('Bulk delete');
    // Also delete descendants of selected tasks
    const toDelete = new Set(this.selectedTasks);
    const addDescendants = (id) => {
      this.tasks.filter(t => t.parentId === id).forEach(c => { toDelete.add(c.id); addDescendants(c.id); });
    };
  ...
  formatDate(ds) {
    if (!ds) return '';
    return this.relativeDate(ds);
  },
  formatDateShort(ds) { return this.formatDate(ds); },
  formatDateAbsolute(ds) {
    if (!ds) return '';
  ...
  dueDateClass(ds) { if (!ds) return ''; const diff = (new Date(ds+'T23:59:59') - new Date()) / 86400000; return diff < 0 ? 'overdue' : diff < 2 ? 'soon' : ''; },
  timeAgo(ts) {
    if (!ts) return '';
  ...
  handleBoardCardClick(event, taskId) {
    ...
    if (event.shiftKey) {
      ...
      if (col) {
        ...
        let lastIdx = -1;
        ids.forEach((id, i) => { if (this.selectedTasks.has(id)) lastIdx = i; });
        if (lastIdx >= 0) {
          const from = Math.min(lastIdx, clickedIdx);
          const to = Math.max(lastIdx, clickedIdx);
          for (let i = from; i <= to; i++) this.selectedTasks.add(ids[i]);
        } else {
